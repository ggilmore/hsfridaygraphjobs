...
object readlargetextfile extends App {
  ...
  for (line <- Source.fromFile("/Users/gmgilmore/Dropbox/ScalaDev/hsfridaygraphjobs/src/resources/connections.txt").getLines) {
    ...
    val name = pair(0)
    val neighbor = pair(1)
    if (nodeNeighborMapping.contains(name)) nodeNeighborMapping(name).add(neighbor) else {
  ...
  val node999 = nameToNodeInstanceMap("999")
  println(bfs(node1, node999, nameToNodeInstanceMap.values))
